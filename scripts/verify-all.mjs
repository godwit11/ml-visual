...
const EXTRA = [
  ...
  /*
   * 「报告问题」浮层。和上面那两条分开，是因为它盯的约束完全不同：
   *   ① **不能和 Nya 重叠** —— 这是当初放弃"右下角"方案的全部理由，
   *      而 Nya 可以拖、尺寸随姿势变，所以必须每次量；
   *   ② 现场信息里**真的带上了页面状态** —— 靠复用 Nya 的状态读取器，
   *      那条链路断掉是**静默的**（不报错、提交照样成功，只是邮件里少几个数）；
   *   ③ 用户取消勾选的那一项**不能进请求体**。
   */
  ['反馈浮层 · 桌面', 'e2e:feedback'],
  ['反馈浮层 · 移动', 'e2e:feedback:mobile'],
  /*
   * Nya 能不能看见「当前这一页」。
   *
   ...
   * 「报告问题」的服务端链路。
   *
   * 和上面那条分开，是因为它盯的东西完全不同：用户输入会进**邮件头**
   * （类型 → 主题），所以有 header injection 的风险；防滥用的四层
   * （蜜罐 / 耗时 / 来源 / 限流）**全在服务端判断**，前端测试再全也验不到；
   * 还有"发信失败时报告必须落进日志"这条 —— 只有在这一层能验。
   */
  ['反馈链路（邮件与防滥用）', 'check:report'],
  /*
   * Nya 的「眼睛」和反馈的「附图」。
   *
   * 两条都是**抓图 → 压缩 → 随请求发出去**，而且都在真浏览器里跑 ——
   * 整条链走的是 ClipboardEvent / canvas.toBlob / toDataURL 这些浏览器专有 API，
   * 在 Node 里一步都测不了，而它们**每一步失败都是静默的**：
   * 图没进来、或者进来了没发出去，界面照样正常，只有用户会觉得"怎么还是看不见"。
   *
   * fetch 都被换掉了，不花额度、结果确定 ⇒ 可以放心进全量回归。
   * ⚠️ 真模型的看图质量另有一条 `npm run nya:image:live`（**会花额度**，刻意不进这里）。
   */
  ['Nya 看图 · 抓图链路', 'e2e:nya:image'],
  ['反馈附图 · 上传链路', 'e2e:feedback:image'],
  /*
   * 「附上页面图表」按钮 —— 一次应该附上**页面上所有可见图表**。
   * 和上面那条分开：那条走**粘贴**（验浏览器专有 API 通不通），
   * 这条走**点按钮**（验抓了几张、顺序对不对）。
   * ⚠️ 这个改动天生不会报错：抓 1 张和抓 3 张界面都正常，
   *    只有按了按钮的人会发现"我想问的那张没附上"。
   */
  ['反馈附图 · 附上页面图表', 'e2e:feedback:shot'],
  /*
   * 图表清单的**锚点对账**：`api/nya.ts` 里 `charts[].title` 必须能在
   * 对应页面的 `.panel-title` 里逐字找到。
   *
   * 为什么单独有这条：title 原来写的是**方位**（"下方左边那张小图"），
   * 那是照着**桌面布局**写的；而 `.chart-row` 在 768px 以下变成单列，
   * 两张小图从"左右并排"变"上下一叠" ⇒ **手机端学生按方位找永远对不上**，
   * 看起来就像"她识图有问题"（2026-09-18 用户报的）。
   * ⇒ 这类 bug 是"文案 vs 布局"的契约，**桌面端任何 e2e 都是绿的**，
   *    只有这条会红。而且它同时会扫「还有没有残留的方位词」。
   */
  ['Nya 图表锚点', 'check:orientation'],
  /*
   * 部署形态检查 —— 复刻 Vercel 的加载环境（编译成独立 .js 后用 ESM 真加载一遍）。
   * 2026-09-15 线上 /api/chat 500 的那两个成因（依赖在 /api 之外、import 漏扩展名）
   * **本地 dev 和 typecheck 都测不出来**：Vite 用 bundler 解析会猜扩展名也不在乎目录。
   * 只有这一条能在推送之前拦住它。它现在会**逐个入口**验（chat + report）。
   */
  ['部署形态（Vercel 能否加载）', 'check:vercel'],
]
